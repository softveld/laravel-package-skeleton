...
author_name=$(ask_question "Author name" "$user_name")

user_email=$(git config user.email)
author_email=$(ask_question "Author email" "$user_email")
...
echo
files=$(grep -E -r -l -i ":author|:vendor|:package|softveld|skeleton" --exclude-dir=vendor ./* ./.github/* | grep -v "$script_name")
...
for file in $files ; do
    ...
    | sed "s/Softveld/$vendor_name/g" \
    | sed "s/OriginalVendor/Softveld/g" \
